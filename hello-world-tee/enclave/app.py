...
def read_identity() -> dict:
    eth_res = requests.get(f"{odyn.endpoint}/v1/eth/address", timeout=10)
    eth_res.raise_for_status()
    eth_identity = eth_res.json()
    encryption_identity = odyn.get_encryption_public_key()
...
async def root():
    ...
    try:
        ...
        logger.exception("Failed to fetch identity from odyn")
        error = str(exc)
    ...
    if error:
        error_block = (
            "<p class='error'>Failed to load identity from odyn: "
            f"{escape(error)}</p>"
        )
