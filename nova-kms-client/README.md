...
1.  **Service Discovery**: Resolves KMS node URLs via `NovaAppRegistry` (`KMS_APP_ID` → ENROLLED versions → ACTIVE instances).
...
- **Automated Testing**: Periodically (every 30s by default; configurable via `TEST_CYCLE_INTERVAL_SECONDS`) picks a random KMS node and runs a full cycle:
    - `/kms/derive` (Key Derivation)
    - `/kms/data` (Write Encrypted Data)
    - `/kms/data/{key}` (Read Encrypted Data)
...
This client discovers KMS nodes via `NovaAppRegistry`. Configure these values in `enclave/config.py` before building/deploying:
...
2.  **Deploy**:
    ...
    Ensure `NOVA_APP_REGISTRY_ADDRESS` and `KMS_APP_ID` are correctly set in `enclave/config.py` in the image you deploy.
